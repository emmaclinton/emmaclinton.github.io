...
FROM flood;
-- rename columns with AS keyword (not a permanent change)
...
--Each Geometry record in PostGIS contains information about its SRID (EPSG code; unique projection #) and geometry
...
SELECT *, st_area(geom)/100000 as area_m2
FROM flood;
...
Gender ratio? Seems like it is rounding up to an integer value
...
SELECT ward_name, count(ward_name) as cntname
from census
group by ward_name
...
SELECT addgeometrycolumn('emmac','ward_census','utmgeom',32737,'MULTIPOLYGON',2);
...
#
ALTER TABLE ward_census
ADD COLUMN areasqkm real;

UPDATE ward_census
SET areasqkm = st_area(utmgeom)/100000;
...
CREATE TABLE flooddissolve -- This creates a dissolved, generalized layer of all flooding
AS
...
You can add the results of this query to the QGIS map using the Load as new layer option (checkbox) at the bottom of the results, and set the Geometry column to geom.
...
SELECT addgeometrycolumn('emmac','ward_fl_area','utmgeom',32737,'MULTIPOLYGON',2);
--the parameters are: schema, table, column, SRID, geometry type, dimensions
...
#
ALTER TABLE ward_flood_area
ADD COLUMN flood_area real;

UPDATE ward_flood_area
SET flood_area = st_area(utmgeom)/100000;
#

#
ALTER TABLE ward_fl_area
ADD COLUMN flood_area_ real;

UPDATE ward_fl_area
SET flood_area = st_area(utmgeom)/100000;
#

/*Now calculate utmgeom for new geometries*/

SELECT addgeometrycolumn('emmac','ward_flood_','utmgeom',32737,'MULTIPOLYGON',2);
UPDATE ward_flood_area
SET utmgeom = ST_Transform(geom, 32737);
...
#
SELECT ward_census.*
FROM ward_census INNER JOIN ward_fl_area
ON wards_census.ward_name = ward_fl_area.ward_name;
#

/* alter table lets to change table structure, including adding, dropping, or renaming columns. int is the integer data type for the columns */

ALTER TABLE ward_census
ADD COLUMN flooa_area real;
/* alter table lets to change table structure, including adding, dropping, or renaming columns. int is the integer data type for the columns */

/* update fields based on a join */

UPDATE ward_census
SET flooa_area = ward_fl_area.flood_area
FROM ward_fl_area
WHERE ward_census.ward_name = ward_fl_area.ward_name;
...
#
ALTER TABLE ward_census
ADD COLUMN pctflood real;

UPDATE ward_census
SET pctflood= (flooa_area/areasqkm)*100;
...
ALTER TABLE ward_census
ADD COLUMN popDens real;

UPDATE ward_census
SET popdens= (totalpop/areasqkm);
